...
pub async fn email_password_login(state: &mut State, email: impl ToString, password: impl ToString, app_name: impl ToString) -> Result<()> {
    ...
    let response = post_request::<LoginSuccessResponse>(state, "/auth/login", body).await?;
